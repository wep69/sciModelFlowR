...
Scientific package validation requires more than successful execution.
It combines known-truth fixtures, independent numerical references,
invariants, artifact integrity, environment capture, and honest
separation between checks actually run and checks still pending.

> This focused vignette uses `gold_linear_regression`, a frozen
...
- distinguish unit, integration, numerical, golden, invariant, and
  simulation tests
...
It combines known-truth fixtures, independent numerical references,
invariants, artifact integrity, environment capture, and honest
separation between checks actually run and checks still pending. A
scientifically strong workflow makes the relevant information boundary
explicit before model fitting. The package therefore represents this
topic as a contract that can be inspected, serialized, tested, and
carried into later modules.
...
For `gold_linear_regression`, this principle can be inspected directly
...
An inappropriate validation strategy is to declare the package validated
because examples render or because a screenshot has not changed. Visual
regression tests can detect presentation changes, but they cannot
establish numerical correctness, coverage, absence of leakage, backend
equivalence, or statistical calibration. Another error is to report a
runtime test as passed when it was never executed in the current
environment.
...
- For statistical simulation tests, use predeclared acceptance bands
  designed to avoid both false confidence and flaky failures.
...
produced it. A later package version may preserve an earlier contract
while changing implementation details, optional backend versions,
numerical defaults, or dependency behavior. For that reason, the final
consolidated validation cycle should execute version-specific fixtures
and record which claims are inherited, which are rechecked, and which
require new numerical evidence. Cross-language agreement should be
evaluated on frozen scientific quantities and tolerances rather than on
incidental object layouts or identical random-number streams. When an
optional backend is unavailable, the corresponding claim remains
unvalidated rather than being inferred from another engine. This
separation between implementation completeness, static audit, numerical
validation, and release certification is part of the scientific
provenance of `sciModelFlowR`.

### Release 0.4 note: validation evidence for adaptive search

Reproducibility of tuning requires more than the selected parameter
vector. Preserve the search-space definition, conditional rules,
candidate or acquisition history, objective directions, inner split
hashes, random seed, budget, failed trials, Pareto set, and any explicit
compromise rule. For nested validation, preserve outer and inner
manifests separately and label inner selection scores separately from
outer generalization estimates. Cross-language validation should compare
frozen configurations and scientific quantities rather than assuming
identical pseudorandom sequences between R and Python.

Validation logs should also record the compute environment used for
adaptive search, because parallelism, backend versions, and stochastic
learners can alter the exact path even when the scientific procedure is
unchanged.

Preserve those differences as provenance rather than silently
