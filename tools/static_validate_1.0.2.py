...
required = [
    ...
    'RELEASE_MANIFEST_1.0.0.json', 'BIBLIOGRAPHY_LEDGER_1.0.0.csv', 'RELEASE_MANIFEST_1.0.2.json'
]
for x in required:
    check((ROOT / 'inst/metadata' / x).exists(), f'metadata {x} exists')

desc = (ROOT / 'DESCRIPTION').read_text(encoding='utf-8')
check(bool(re.search(r'(?m)^Version: 1\.0\.2$', desc)), 'DESCRIPTION version 1.0.2')
check((ROOT / 'NEWS.md').read_text(encoding='utf-8').startswith('# sciModelFlowR 1.0.2'), 'NEWS begins 1.0.2')
check('version: 1.0.2' in (ROOT / 'CITATION.cff').read_text(encoding='utf-8'), 'CITATION.cff version 1.0.2')
check('R package version 1.0.2' in (ROOT / 'inst/CITATION').read_text(encoding='utf-8'), 'inst/CITATION version 1.0.2')

nbs = list((ROOT / 'inst/notebooks/jupyter').glob('*.ipynb'))
for p in nbs:
    try:
        o = json.load(open(p, encoding='utf-8'))
        good = o.get('metadata', {}).get('sciModelFlowR', {}).get('release') == '1.0.2'
    except Exception:
        good = False
    check(good, f'Notebook {p.name}: release metadata 1.0.2')
...
out = ROOT / 'outputs' / 'final-reports' / 'STATIC_AUDIT_1.0.2.md'
out.parent.mkdir(parents=True, exist_ok=True)
lines = [
    '# sciModelFlowR 1.0.2 static audit (patch release of the 1.0.0 freeze)', '',
...
out.write_text('\n'.join(lines) + '\n', encoding='utf-8')
print(f'STATIC VALIDATION sciModelFlowR 1.0.2: {len(checks)-len(failed)}/{len(checks)} passed')
