# sciModelFlowR 1.0.2 — release-engineering gate.
# The frozen 1.0.0 release metadata remain the historical reference; this gate
# additionally requires the 1.0.2 release manifest and confirms that the
# public API freeze (0.9.0 -> 1.0.0 -> 1.0.2) is unchanged.
cat("sciModelFlowR 1.0.2 release-engineering checks\n")
...
required <- c(
  ...
  "inst/metadata/RELEASE_MANIFEST_1.0.2.json"
)
...
m <- jsonlite::fromJSON(
  file.path(root, "inst/metadata/RELEASE_MANIFEST_1.0.2.json"),
  simplifyVector = TRUE
)
stopifnot(identical(m$version, "1.0.2"))
...
cat("Release metadata contract checks complete for 1.0.2.\n")
