...
smf_run_gold_validation <- function(stop_on_failure=FALSE) {
  manifest_path <- system.file("gold_data","gold_hashes.csv",package="sciModelFlowR")
  if(!nzchar(manifest_path)) manifest_path <- file.path("inst","gold_data","gold_hashes.csv")
  man <- utils::read.csv(manifest_path,stringsAsFactors=FALSE)
  res <- lapply(seq_len(nrow(man)), function(i) {
    nm <- man$dataset[[i]]
    path <- system.file("extdata","gold_data",paste0(nm,".csv"),package="sciModelFlowR")
    if(!nzchar(path)) path <- file.path("inst","extdata","gold_data",paste0(nm,".csv"))
