...
.smf_tune_racing <- function(spec,data,space,tuning,resamples,objectives) {
  ...
  for (f in seq_along(resamples@splits)) {
    for (i in active) {
      ...
      hist[[length(hist)+1L]] <- .smf_archive_row(as.list(cfgs[i,,drop=FALSE]),v,length(hist)+1L,NA_real_,list(stage=as.integer(f),n_folds=1L,status="active"))
    }
  ...
  # Aggregate rows share the per-fold schema (stage=NA) so the final rbind is
  # homogeneous (derived fix 1.0.0+).
  agg <- lapply(seq_len(nrow(cfgs)),function(i){ z<-fold_values[[i]]; if(is.null(z)||!nrow(z))return(NULL); vals=tapply(z$value,z$metric,mean,na.rm=TRUE); .smf_archive_row(as.list(cfgs[i,,drop=FALSE]),vals,100000L+i,NA_real_,list(stage=NA_integer_,n_folds=length(unique(z$fold)),status=if(i%in%active)"survivor" else "eliminated")) })
